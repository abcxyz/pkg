...
class TeamMembers {
  ...
  async contains(login) {
    try {
      ...
      if (err.status === 404) {
        core.debug(`Received 404 testing membership; assuming user is not a member: ${JSON.stringify(err)}`);
...
async function validate({team, prNumber, repoName, repoOwner, octokit}) {
  const members = new TeamMembers(repoOwner, team, octokit);
  const prResponse = await octokit.rest.pulls.get({owner: repoOwner, repo: repoName, pull_number: prNumber});
  ...
  const submittedReviews = await octokit.paginate(octokit.rest.pulls.listReviews, {
    owner: repoOwner,
    repo: repoName,
 ...
 */
async function revalidate({workflowId, repoName, repoOwner, branch, prNumber, octokit}) {
  // Get all failed runs.
...
async function main() {
  try {
    ...
    if (eventName === PULL_REQUEST_REVIEW) {
      const workflowId = await getWorkflowId(octokit, repoOwner, repoName, runId);
      await revalidate({workflowId, branch, repoName, repoOwner, prNumber, octokit});
    }
  } catch (err) {
